...
<%
	...
	try {
		...
		while (rs.next()) {
			sd.setAccountId(rs.getLong("accountid")); //계정아이디
			sd.setId(rs.getLong("summonerid")); //소환사아이디
	...
	try {
		...
		pstmt = dbcon.prepareStatement(sql);
		pstmt.setLong(1, sd.getId()); //소환사아이디로 쿼리문받기
		rs = pstmt.executeQuery();
		while (rs.next()) {
			sd.setRank(rs.getString("rank")); //소환사 랭크
